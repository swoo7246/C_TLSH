...
| **merge.py** | window + dike 병합 | sample_diff_win_* 와 sample_diff_dike_* 결과 디렉토리를 비교하여 동일 함수의 결과 중 더 낮은 min_diff 값을 선택. (window와 dike의 min_diff를 비교해서 더 낮은 min_diff를 남김)

| **decompile.py** | C언어로 디컴파일 | 디컴파일 후 extract_integrated_dotenv.py로 txt 파일로 변환. preprocess_all.py는 extract된 모든 txt파일을 preprocess후 jsonl 파일로 저장.<br>예시: ```{"Function Name":"FUN_004011b0","Address":"004011b0","Source Code":"int FUN_004011b0(undefined8 param_1,undefined8 param_2,ulonglong param_3)", ...}```
